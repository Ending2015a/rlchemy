...
from rlchemy.lib.envs.vec import subproc as rl_subproc
...
class TestSubprocVecEnvModule(TestCase):
  ...
  def test_subproc_vec_env(self):
    num_envs = 4
    env_fns = [
      functools.partial(create_fake_env, i, 'Box')
      for i in range(num_envs)
    ]
    envs = rl_subproc.SubprocVecEnv(env_fns)
    # test properties
  ...
  def test_subproc_vec_env_obs_norm(self, space_type):
    num_envs = 4
    env_fns = [
