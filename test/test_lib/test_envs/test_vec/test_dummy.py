...
from rlchemy.lib.envs.vec import dummy as rl_dummy
...
def create_atari_env(env_id):
  env = rl_atari.make_atari(env_id)
  env = rl_atari.wrap_deepmind(env)
  return env

class TestDummyVecEnvModule(TestCase):
  def test_dummy_vec_env(self):
    num_envs = 4
    env_fns = [
      functools.partial(create_fake_env, i, 'Box') 
      for i in range(num_envs)
    ]
    envs = rl_dummy.DummyVecEnv(env_fns)
    # test properties
  ...
  def test_dummy_vec_env_various(self, space_type):
    num_envs = 4
    env_fns = [
  ...
  def test_dummy_vec_env_image(self):
    num_envs = 4
    env_fns = [
  ...
  def test_vec_env_various(self, space_type):
    num_envs = 4
    envs = [
      create_fake_env(i, space_type)
      for i in range(num_envs)
    ]
    envs = rl_dummy.VecEnv(envs)
    self.assertEqual(len(envs), num_envs)
    envs.seed(1)
    obs = envs.reset()
    self.assertArrayEqual(obs.shape, (num_envs, *envs.observation_space.shape))
